...
import { Product, CollectionBlock } from '@/types'
...
export default function NewCollectionPage() {
  ...
  const [bgImage, setBgImage] = useState<string | null>(null) // Para banner opcional
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const { data } = await supabase
        .from('products')
        .select('id, name, images, category, status')
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from('products') // Usando mesmo bucket por simplicidade
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          content,
          display_order: 0 // Default, depois implementamos reordenação
        })
      ...
      alert('Erro ao criar coleção. Verifique se o slug já existe.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="p-4 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-xl font-display font-semibold">Nova Coleção</h1>
          <p className="text-sm text-brand-500">Crie uma curadoria de produtos</p>
        </div>
      </div>
            ...
            <div className="bg-white rounded-2xl p-4 border border-brand-100 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  readOnly // Auto-generated for simplicity
                />
              </div>
